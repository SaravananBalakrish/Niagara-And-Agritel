...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  int _otpRequestCount = 0; // Track OTP requests
  static const int _maxOtpRequests = 3; // Max requests before rate limiting
  ...
  AuthBloc({
    ...
    on<SendOtpEvent>((event, emit) async {
      ...
      // Check rate limiting
      if (_otpRequestCount >= _maxOtpRequests &&
          _lastOtpRequestTime != null &&
          DateTime.now().difference(_lastOtpRequestTime!).inMinutes < 2) {
        emit(AuthError(
